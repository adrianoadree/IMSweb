...
function RecordQuickView(props) {
  const [recordDoc, setRecordDoc] = useState(); // transaction document id
  ...
  const [supplierCollection, setSupplierCollection] = useState(); // supplier collection
  ...
  // check if record data is fetched
  useEffect(() => {

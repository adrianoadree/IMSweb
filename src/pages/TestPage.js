...
import { LineChart, Line, XAxis, YAxis, ReferenceDot, Legend, Tooltip, ReferenceLine } from 'recharts';
...
function TestPage() {
                            ...
                            <Accordion.Item eventKey="0">
                                ...
                                <Accordion.Body>
                                    ...
                                    >
                                        ...
                                        <Tooltip />
                                        <Legend />
                                        <ReferenceDot x={moment(fillerDate4).format('ll')} y={reorderPoint} shape={CustomReferenceDot} label="ReorderPoint" />

                                        <Line type="monotone" dataKey="SalesTransaction" stroke="#8884d8"
                                            dot={{ stroke: '#8884d8', strokeWidth: 1, r: 10, strokeDasharray: '' }}
                                        />

                                        <Line type="monotone" dataKey="ProjectedSales" stroke="blue"
    ...
    useEffect(() => {
        ...
        tempf1 = tempDate1.setDate(tempDate1.getDate() + quarterROP)
        tempf2 = tempDate2.setDate(tempDate2.getDate() + halfROP)
        tempf3 = tempDate3.setDate(tempDate3.getDate() + (halfROP + quarterROP))
        tempf4 = tempDate4.setDate(tempDate4.getDate() + daysROP)
        ...
        console.log("HalfROP: ", halfROP)
        console.log("quarterROP: ", quarterROP)
        console.log("fillerDate4: ", fillerDate4)
    ...
    useEffect(() => {
        ...
        let z = 0
        z = x - y
        setCurrQuantityROPDifference(Math.round(z))
        let halfQuantity = Math.round(currQuantityROPDifference/2)
        let quarterQuantity = Math.round(currQuantityROPDifference/4)

        let tempQuantity1 = q1Quantity;
        let tempQuantity2 = q1Quantity;
        let tempQuantity3 = q1Quantity;
        let tempQuantity4 = q1Quantity;
 
        setFillerQuantity1(Math.round(tempQuantity1 - quarterQuantity))
        setFillerQuantity2(Math.round(tempQuantity2 - halfQuantity))
        setFillerQuantity3(Math.round(tempQuantity3 - (halfQuantity+ quarterQuantity)))
        setFillerQuantity4(Math.round(tempQuantity4 - currQuantityROPDifference))
    ...
    const data = [
        {
            ...
            SalesTransaction: q5Quantity,
        },
        {
            date: moment(q4Date).format('ll'),
            SalesTransaction: q4Quantity,
        },
        {
            date: moment(q3Date).format('ll'),
            SalesTransaction: q3Quantity,
        },
        {
            date: moment(q2Date).format('ll'),
            SalesTransaction: q2Quantity,
        },
        {
            date: moment(q1Date).format('ll'),
            SalesTransaction: q1Quantity,
            ProjectedSales: q1Quantity
        },
        {
            date: moment(fillerDate1).format('ll'),
            ProjectedSales: fillerQuantity1,
        },
        {
            date: moment(fillerDate2).format('ll'),
            ProjectedSales: fillerQuantity2,
        },
        {
            date: moment(fillerDate3).format('ll'),
            ProjectedSales: fillerQuantity3,
        },
        {
            date: moment(fillerDate4).format('ll'),
            ProjectedSales: reorderPoint,
        },
